...
function calculateBQI(aqi, mq135Score, humidityScore, temperatureScore, spo2Score) {
  return Math.round(
    (aqi * 0.50) +
    (mq135Score * 0.25) +
    (humidityScore * 0.10) +
...
app.post("/api/sensor-data", async (req, res) => {
  try {
    ...
    const mq135Score = getMQ135Score(mq135);
    const humidityScore = getHumidityScore(humidity);
    const temperatureScore = getTemperatureScore(temperature);
    const spo2Score = getSpo2Score(spo2);
    const bqi = calculateBQI(aqi, mq135Score, humidityScore, temperatureScore, spo2Score);
    const category = getCategory(bqi);
    const riskLevel = getRiskLevel(bqi);
...
setInterval(async () => {
  try {
    ...
    const avgTemperature = minuteBuffer.reduce((s, i) => s + i.temperature, 0) / minuteBuffer.length;
    const avgHumidity = minuteBuffer.reduce((s, i) => s + i.humidity, 0) / minuteBuffer.length;
    const avgMQ135 = minuteBuffer.reduce((s, i) => s + i.mq135, 0) / minuteBuffer.length;
    const avgDust = minuteBuffer.reduce((s, i) => s + i.dust, 0) / minuteBuffer.length;
    const avgSpo2 = minuteBuffer.reduce((s, i) => s + i.spo2, 0) / minuteBuffer.length;
    const avgHeartRate = minuteBuffer.reduce((s, i) => s + i.heartRate, 0) / minuteBuffer.length;

    const aqi = calculateAQI(avgDust);
    const bqi = calculateBQI(
      aqi,
    ...
    const reading = new Reading({
      ...
      mq135: Number(avgMQ135.toFixed(2)),
      dust: Number(avgDust.toFixed(2)),
      spo2: Number(avgSpo2.toFixed(2)),
      heartRate: Number(avgHeartRate.toFixed(2)),
      aqi,
      bqi,
      category: getCategory(bqi),
      riskLevel: getRiskLevel(bqi)
    });
